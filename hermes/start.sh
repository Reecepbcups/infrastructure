...
FOLDER="/root/hermes-notes"
HERMES="hermes --config /root/hermes-notes/config.toml"
...
$HERMES keys add --chain provider --key-name relayer0 --mnemonic-file $FOLDER/relayer-mnemonic || true

# Create key in hermes for wasmd-1 chain with the name relayer1
$HERMES keys add --chain apollo --key-name relayer1 --mnemonic-file $FOLDER/relayer-mnemonic || true

# Create key in hermes for osmo-testing chain with the name relayer2
$HERMES keys add --chain sputnik --key-name relayer2 --mnemonic-file $FOLDER/relayer-mnemonic || true

# Lets start a connection between our 2 chains, make sure the chains are running or else this will not work.
$HERMES create connection --a-chain provider --b-chain apollo;
# $HERMES create connection --a-chain apollo --b-chain sputnik;
